...
class OptimizationProblem:
    ...
    def __init__(self, client_df, fund_df, model_df, ac_len, fund_len):
        ...
        self.fund_df = fund_df.iloc[:, 2:].set_index(self.fund_list)
        ...
        self.ac_list = model_df.columns[1:]
    ...
    def export_fund_output(self):
        ...
        fund_allocation_output = {
            'Fund Name': [fund for fund in self.fund_list],
            ...
            'Txn Count': [int(txn.value()) for txn in self.y_vars]
        }

        fund_allocation_output = pd.DataFrame(fund_allocation_output).to_json(orient='records')

        return fund_allocation_output
