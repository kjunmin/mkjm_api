...
def optimize_portfolio(input_data):
    client_df = pd.DataFrame.from_dict(input_data['clientJson'])
    model_df = pd.DataFrame(input_data['modelJson'], index=[0])
    fund_df = pd.DataFrame.from_dict(input_data['fundJson'])

    fund_len = len(fund_df['Fund Name'])
    ...
    op.solve_problem()
    fund_out_json = op.export_fund_output()


    return(fund_out_json)
